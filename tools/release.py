...
and builds the self-contained WebUI release file. The former standalone build
scripts now delegate to this file so release generation has a single entry point.
"""
...
def build_webui(
    ...
    *,
    ...
    debug_print(debug, f'webui minify: {minify}')
...
def build_release(args: argparse.Namespace) -> int:
    ...
    build_webui(args.lang, expected, minify=not args.no_minify, debug=debug)
...
def build_webui_cli(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument('--debug', action='store_true')
    args = parser.parse_args(argv)
    build_webui(args.lang, args.out, minify=not args.no_minify, debug=debug_enabled(args.debug))
    return 0
